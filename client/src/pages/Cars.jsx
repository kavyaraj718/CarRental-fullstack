...
const Cars = () => {
  //  getting search panels form url
  ...
  // Initialize filteredCars when cars data arrives
useEffect(() => {
  if (cars.length > 0) {
    setFilteredCars(cars);
  }
...
// Run filter whenever input changes
useEffect(() => {
  if (input.trim() === '') {
    setFilteredCars(cars);
  } else {
    ...
    <div>
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: 'easeOut' }} className='flex flex-col items-center py-20 bg-light max-md:px-4'>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className='flex items-center bg-white px-4 mt-6 max-w-140 w-full h-12 rounded-full shadow'>
          ...
          <input onChange={(e) => setInput(e.target.value)} value={input} type="text" placeholder='Search by brand, model or location' className='w-full h-full outline-none text-gray-500' />
